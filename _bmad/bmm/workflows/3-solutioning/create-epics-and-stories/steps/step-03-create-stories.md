...
Before processing epics, check for batch opportunity:

If the approved epics_list contains **4 or more epics**, present this option to the user:

```
TEAM MODE AVAILABLE

There are {{epic_count}} epics to decompose into stories. I can process them in two ways:

1. **Agent Team** — I'll create a team where each teammate decomposes ~2 epics in parallel.
   Each teammate follows the same story creation process and you can interact with any
   teammate via Shift+Down. The lead synthesizes and validates at the end.
   Uses more tokens but finishes faster.

2. **Sequential** (default) — I process each epic one at a time in this session.

Choose [1] or [2]:
```

If user chooses Agent Team:
- Read and follow the Agent Team Protocol at `{project-root}/_bmad/bmm/workflows/agent-team-protocol.md`
- Use workflow_name="create-epics-and-stories" and the create-epics-and-stories team configuration
- Each teammate gets assigned epics and follows sections 3-5 below for their epics
- The lead merges outputs into {outputFile} and runs section 6 (Final Document Completion)
- Then proceed to section 7 (Present FINAL MENU OPTIONS)

If user chooses Sequential or does not respond, continue below as normal.
