...
Before executing, check for multi-lens review opportunity:

<team-mode-check>
  <action>Present the team review option to {user_name}:</action>
  <output>
  REVIEW MODE

  I can review this story in two ways:

  1. **Multi-Lens Agent Team** — 3 reviewers working in parallel, each with a different focus:
     - Security and data integrity
     - Performance, resource management, and architecture compliance
     - Correctness, acceptance criteria, and test coverage
     They challenge each other's findings and produce a unified review. Uses more tokens.

  2. **Single Reviewer** (default) — I review everything sequentially in this session.

  Choose [1] or [2]:
  </output>
  <check if="user chooses 1 (Agent Team)">
    <action>Read and follow the Agent Team Protocol at {project-root}/_bmad/bmm/workflows/agent-team-protocol.md</action>
    <action>Use workflow_name="code-review" and the code-review team configuration</action>
    <action>Create 3-teammate review team with the lenses described above</action>
    <action>Each teammate follows this workflow's EXECUTION steps but with their specific review focus</action>
    <action>Lead synthesizes all findings into a unified review report</action>
    <action>HALT this workflow — team handles execution</action>
  </check>
  <check if="user chooses 2 or no response">
    <action>Continue with normal EXECUTION below</action>
  </check>
</team-mode-check>
