...
<team-mode-check>
  ...
  <action>Count all stories with status "backlog" in development_status</action>
  <check if="backlog_count >= 3 AND no specific story was requested by user">
    <action>Read and follow the Agent Team Protocol at {project-root}/_bmad/bmm/workflows/agent-team-protocol.md</action>
    <action>Use workflow_name="create-story" and the create-story team configuration</action>
    <action>Set batch_items to the list of backlog story keys</action>
