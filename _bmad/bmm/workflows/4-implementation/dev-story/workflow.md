...
Before executing, check for batch opportunity:

<team-mode-check>
  <action>Load {sprint_status} if it exists</action>
  <action>Count all stories with status "ready-for-dev" in development_status</action>
  <check if="ready_for_dev_count >= 2 AND no specific story was requested by user">
    <action>Read and follow the Agent Team Protocol at {project-root}/_bmad/bmm/workflows/agent-team-protocol.md</action>
    <action>Use workflow_name="dev-story" and the dev-story team configuration</action>
    <action>Set batch_items to the list of ready-for-dev story keys</action>
    <action>Present the team option to {user_name}</action>
    <check if="user chooses Agent Team or Subagents">
      <action>Follow the protocol's Step T3 for the chosen option</action>
      <action>CRITICAL: Check story file lists for overlapping files before assigning parallel work</action>
      <action>Stories touching the same files MUST be assigned to the same teammate sequentially</action>
      <action>Each teammate/subagent follows this workflow's EXECUTION steps independently for their assigned stories</action>
      <action>HALT this workflow — team handles execution</action>
    </check>
    <check if="user chooses Sequential">
      <action>Continue with normal EXECUTION below (one story at a time)</action>
    </check>
  </check>
</team-mode-check>
